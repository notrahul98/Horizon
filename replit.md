...
```
Two pages: `/` is the overview (KPIs, sector/movers/rotation charts, sortable/searchable stock table); clicking a row goes to `/stock/<symbol>`, a full-width page with Technical / Fundamental / Corporate / Company / AI Signal / **Analysts & News** tabs (previously all four were crammed into one 380px side panel alongside the table on a single page). `dashboard.py` also registers the Phase 2 `corporate_bp` blueprint from `corporate_dashboard.py`, though note the actual `/api/corporate/<symbol>` route the frontend calls is a separate live-yfinance implementation inside `dashboard.py` itself, not the blueprint's `/api/corporate/stock/<symbol>` — the blueprint's routes exist but nothing in the UI calls them.

**Analysts & News tab** (`/api/analysts/<symbol>`): aggregate analyst recommendation counts (strongBuy/buy/hold/sell/strongSell, weighted into a 1-5 consensus score), mean/high/low/median price targets with computed upside vs current price, and a recent-news feed — all live from yfinance, nothing stored. Deliberately does **not** claim per-broker attribution ("Morgan Stanley says X") — `ticker.upgrades_downgrades` (the field that would carry that) came back empty for every Indian stock tested (RELIANCE, TCS, INFY, ETERNAL, IRCTC), so it's just not populated for NSE tickers in Yahoo's data. The UI banner says so explicitly rather than implying detail that isn't there.

**Live price on the detail page** (`/api/live-price/<symbol>`): polls `yfinance`'s `fast_info` (a cheap handful of fields, not the full `.info` payload) every 30s while a stock's detail page is open, updating just the price/day-change display in place. Deliberately scoped to whichever single stock is currently open, not all 150 at once — yfinance is unofficial/delayed data, not a real streaming feed, and polling the whole watchlist this frequently would risk rate-limiting. Client-side JS only starts polling during NSE market hours (weekdays, ~9:00-15:35 IST) and pauses via the Page Visibility API when the tab isn't focused, so it isn't running client-side JS unnecessarily. True tick-by-tick live data would need a paid provider (a broker API like Zerodha Kite Connect / Upstox, or a real market-data vendor) — a bigger, costed decision, not what this is.
...
**Phase 5 — daily report (16:10 IST weekdays, 10 min after the price refresh)**: `scanner.py` runs a rule-based swing-trade screen (same style as the Phase 4 agents' offline heuristics, not a prediction model) over every tracked stock — AI consensus BUY with HIGH/MEDIUM conviction and ≥55% confidence, trend alignment (close > EMA20 > EMA50), RSI < 70 — and for anything that clears it, derives a stop-loss/target from ATR(14) (stop = entry − 1.5×ATR, target = entry + 2.5×risk).

**Delivery: displayed on `/report` for now, not Telegram.** `notifications.py` has working Telegram send logic, but Telegram is blocked in the user's country, so it's not wired into the active delivery path — `_run_daily_alert()` only calls `send_telegram()` if `telegram_enabled()` (i.e. only if someone sets the env vars anyway). The generated report (candidates + breadth snapshot) is cached in the `_last_report` global in `dashboard.py` and rendered at `/report`, linked from the main dashboard nav ("Daily Report →"). `_generate_report_data()` is the single source of truth both paths use, so adding a different channel later (Discord webhook, email, WhatsApp Business API — whatever turns out to be accessible) only means changing what happens with that dict, not the scanning logic itself. To activate Telegram anyway (e.g. via VPN, or if restrictions change):
...
**Live prices on `/trades`**: open positions' "Current ₹" and P&L are priced off yfinance's live `fast_info` (same call as `/api/live-price`) instead of yesterday's stored close, whenever the market is open — `_is_market_hours_ist()` gates it server-side (no point calling Yahoo for a "live" price outside 9:00–15:35 IST weekdays; the stored close is already that same number then) and `_live_price_map()` in `dashboard.py` fetches the whole set of open-trade symbols concurrently via a `ThreadPoolExecutor` (open positions accumulate over time as scanner picks sit until they hit stop/target, unlike the always-one-symbol detail-page ticker, so this can't stay sequential). Any symbol whose live fetch fails just falls back to the last stored close — never a hard error. Each trade row carries a `price_is_live` flag; the UI shows a blinking green dot next to any row currently priced live, plus a page-level "● live prices" badge next to "Open Positions" when at least one row is live. The page itself re-polls `/api/trades` every 30s during market hours (paused when the tab is hidden), mirroring the detail page's `isMarketHours()`/`visibilitychange` pattern. Closing a trade without an explicit exit price (`POST /api/trades/<id>/close`) also prefers the live price over the stored close when the market's open.

**Selection reasoning on `/report`**: each scanner candidate carries a `reason_summary` (one paragraph covering the AI vote split, trend alignment, RSI headroom, and the ATR math behind the stop/target) plus the raw `agent_details` from the consensus call (each of Claude/Gemini/DeepSeek's individual signal, confidence, and reasoning string) — both built in `scanner.py`'s `_build_reason_summary()` and threaded through `scan_for_candidates()`'s output. On `/report`, a "Why?" button per row (event-stopped so it doesn't also trigger the row's click-to-stock-detail navigation) expands a row showing the summary paragraph and a card per agent. The Telegram/text report (`format_daily_report()`) gets the same `reason_summary` as an italic line under each bullet, so the "why" travels with the pick on every delivery channel, not just the web page.
...
- **Phase 4's "AI Signal" agents: Gemini has a real live-API path; Claude/DeepSeek are still offline-only.** All three agents (`stock-collector/agents/`) share a rule-based `_offline_analysis()` using EMA20/50/200, RSI, and Bollinger position — real signal from real indicators. `GeminiAgent` additionally implements a live call: set `GEMINI_API_KEY` in Railway (free key from [Google AI Studio](https://aistudio.google.com/apikey) — the free tier is enough) and it calls the Gemini REST API (`gemini-2.5-flash-lite` by default, override with `GEMINI_MODEL`) via plain `requests` — deliberately no `google-genai` package, so `uv.lock` stays untouched (same pattern as `trades_store.py`). Free-tier protections built in: calls are throttled to one per ~4s module-wide (the daily scanner runs consensus over all 150 symbols — unthrottled that would blow the requests-per-minute cap; throttled, the 16:10 scan takes ~10 min in its background thread, which is fine), a 429 triggers a 60s cooldown, and **any** live-call failure falls back to the offline heuristic with a `[Gemini API unavailable — offline heuristic]` prefix on the reasoning so the UI never passes rule-based output off as a model response. `ClaudeAgent`/`DeepSeekAgent` `analyze()` methods are still stubs — setting `ANTHROPIC_API_KEY`/`DEEPSEEK_API_KEY` flips their `enabled` flag (and the UI banner) but the actual API call code hasn't been written; note consumer subscriptions (claude.ai Pro, the Gemini app) do **not** include API access, which is why Gemini (free API tier) went first.
