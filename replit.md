...
```
`dashboard.py` is the main Flask app (price charts, 4 tabs); it registers the Phase 2 `corporate_bp` blueprint from `corporate_dashboard.py` for the `/api/corporate/*` routes. Corporate data must be populated first via `--corporate` before those routes return anything.
